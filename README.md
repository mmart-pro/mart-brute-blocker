...
```
Доступные команды:
* `allow ip/bitmask` - Добавляет подсеть в белый список
* `deny ip/bitmask` - Добавляет подсеть в чёрный список
* `check ip` - Проверяет включение ip в один из списков.
* `remove ip/bitmask` - Удаляет подсеть из чёрного или белого списка по точному совпадению.
* `clearbucket ip login` - Обнуляет лимит по указанным ip и login
* `help` - Помощь по команде

Флаги:
* `-c, --config string` - json-файл конфигурации
* `-h, --help` - help for mbb-cli
* `--host string` - адрес сервера
* `--port int` - порт сервера
* `-v, --version` - версия приложения

```bash
mbb-cli [command] --help
```
